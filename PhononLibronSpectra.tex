...
\[ \w^2 \pmtrx{\tilde u_x \\ \tilde u_y \\ \tilde \th} = \pmtrx{4A \g_{xx}^2 & 4A \g_{xy}^2 & -2i B \g_{x}^2 \\ 4A \g_{xy}^2 & 4A\g_{yy}^2 & - 2i B \g_{y}^2 \\ 2i B \g_{x}^2 & 2i B\g_{y}^2 & 4D \mu^2 + 4 E \g^2} \pmtrx{\tilde u_x \\ \tilde u_y \\ \tilde \th}.\]
Here, I've defined:
\[ \g_{ij}^2 = \half{1}\sum_{\bv c} \hat{c}_{i}\hat{c}_{j} \sin^2\paren{\half{\Dot k c}},\quad \g_{i}^2 = \half{1} \sum_{\bv c} \hat{c}_i \sinp{\Dot k c},\]
\[ \g^2 = \half{1}\sum_{\bv c} \sin^2\paren{\half{\Dot k c}},\quad \mu^2 = \half{1} \sum_{\bv c} \cos^2 \paren{\half{\Dot k c}}.\]
...
\[ \bv E(\bv k) = \pmtrx{4A \g_{xx}^2 & 4A \g_{xy}^2 & -2i B \g_{x}^2 \\ 4A \g_{xy}^2 & 4A\g_{yy}^2 & - 2i B \g_{y}^2 \\ 2i B \g_{x}^2 & 2i B\g_{y}^2 & 4D \mu^2 + 4 E \g^2},\]
...
In high dimensions, this convergence is slow, but for $d=2$, what we're looking at here, isn't terrible. So we're justified in dropping these terms from the free energy. Continuing on, we can take the sum over $\bv k$ to an integral with the continuum limit
\[ \sum_{\bv k} \goto \frac{V}{(2\pi)^2} \int d\bv k.\]
\[ F = \frac{k_BT V}{8\pi^2} \int d \bv k \, \log \paren{\lam^2 \b \det E_{\bv k}}.\]
...
So now that we've gotten a good handle on the mechanics of the calculation, we can skip right to the part where we plug in the dispersion matrix. For our model of phonons interacting with librons, the determinant of our matrix is
...
Furthermore, the first Brillouin zone is the regular hexagon centered at $\bv k = 0$ whose side lengths are equal to $\pi/c$.
