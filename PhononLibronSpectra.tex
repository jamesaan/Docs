...
\[\mathcal{H} = \half{1} \bv p \cdot \bv M^{-1} \cdot \bv p + \half{1} \bv x \cdot \bv K \cdot \bv x = \half{1}p_i M_{ij} p_j + \half{1}x_i K_{ij}x_j.\]
...
\[ \bv E(\bv k) = \pmtrx{4A \g_{xx}^2 & 4A \g_{xy}^2 & -2i B \g_{x}^2 \\ 4A \g_{xy}^2 & 4A\g_{yy}^2 & - 2i B \g_{y}^2 \\ 2i B \g_{x}^2 & 2i B\g_{y}^2 & 4D \mu^2 + 4 E \g^2}\pmtrx{\tilde u_x \\ \tilde u_y \\ \tilde \th},\]
...
Better yet, we can instead just make the correlation function matrix:
\[ \bv C(\bv k) = \pmtrx{\avg{\tilde u_x^2} & \avg{\tilde u_x \tilde u_y} & \avg{\tilde u_x \tilde \th} \\ \avg{\tilde u_x \tilde u_y} & \avg{\tilde u_y^2} & \avg{\tilde u_y \tilde \th} \\ \avg{\tilde u_x \tilde \th} & \avg{\tilde u_y \tilde \th} & \avg{\tilde \th^2} }.\]
From here, the inverse of this matrix should give the coupling constant matrix!
\[ k_BT \bv C^{-1}(k) = \bv E(k).\]
